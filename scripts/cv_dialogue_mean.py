...
OUTPUT_DIR = PROJECT_ROOT / "results" / "cv_dialogue_mean"
...
def main() -> None:
    ...
    print("\n── Pre-computing all embeddings ──")
    # Pre-calculate once so we don't do it 15 times!
    for iv in all_interviews:
        utts = iv["utterances"]
        if not utts:
            iv["embedding"] = torch.zeros(HIDDEN_DIM)
        else:
            utt_embs = encode_utterances(
                utts, tokenizer, encoder, device,
                max_length=MAX_TOKEN_LENGTH, batch_size=ENCODING_BATCH_SIZE
            )
            iv["embedding"] = mean_pool_interview(utt_embs)

    # Wrapper to inject device into our callback
    def _cv_callback(train_pool, test_set, seed):
        """Callback for run_monte_carlo_cv.
        
        1. Splits train_pool into internal train and dev for early stopping.
        2. Trains the classifier.
        3. Evaluates on test_set strictly.
        """
        # ── 1. Create inner validation split ──
        labels_pool = [iv["label"] for iv in train_pool]
        inner_cv = StratifiedShuffleSplit(n_splits=1, test_size=0.15, random_state=seed)
        ...
        # ── 2. Get pre-computed embeddings ──
        X_train = torch.stack([iv["embedding"] for iv in inner_train])
        X_dev = torch.stack([iv["embedding"] for iv in inner_dev])
        X_test = torch.stack([iv["embedding"] for iv in test_set])
        
        y_train = torch.tensor([iv["label"] for iv in inner_train], dtype=torch.float32)
        y_dev = torch.tensor([iv["label"] for iv in inner_dev], dtype=torch.float32)
        y_test = torch.tensor([iv["label"] for iv in test_set], dtype=torch.float32)
