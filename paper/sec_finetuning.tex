...
positive), \emph{now including the frozen control}. \textbf{OOF AUC} with a
subject-resampling bootstrap 95\% CI; the development per-seed AUC and the
resulting ranks are repeated for contrast. The frozen control is the pooled-OOF
best. Wilcoxon signed-rank on the 15 paired per-run ($5$ fold $\times\,3$ seed)
AUCs finds \emph{no} fine-tuning finalist different from frozen (smallest raw
$p=0.39$; all Holm-corrected $p\!\approx\!1$): no adaptation lift survives the
leakage-free probe.}
\label{tab:ft-oof}
\centering\small
\setlength{\tabcolsep}{4pt}
...
\toprule
\textbf{Model} & \textbf{OOF AUC} & \textbf{95\% CI} & \textbf{dev AUC} &
\textbf{dev rk} & \textbf{OOF rk} \\
\midrule
\textbf{frozen (control)}      & \textbf{0.790} & [.709,.866] & 0.892 & 5 & \textbf{1} \\
last-4, lr1e-5                 & 0.789 & [.706,.862] & 0.908 & 4 & 2 \\
LoRA $r$16+FFN, lr1e-4         & 0.775 & [.687,.851] & 0.915 & 2 & 3 \\
mxbai LoRA $r$16, lr1e-4       & 0.768 & [.685,.843] & 0.916 & \textbf{1} & 4 \\
LoRA $r$8, lr2e-4              & 0.764 & [.676,.846] & 0.913 & 3 & 5 \\
\bottomrule
\end{tabular}
\end{table}
