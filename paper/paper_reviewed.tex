...
% Two-encoder ensemble and encoder fine-tuning extracted to sec_ensemble.tex /
% sec_finetuning.tex and out of scope for this draft.
...
and evaluation protocols that are not comparable across papers and frequently
...
The honest standing of this paper is therefore that we lead the cleanest
\emph{verified single-model} test-set baseline \cite{milintsevich2023} and match
the best reported text-only single model within seed variation
(\Cref{sec:external}). Numerous further results
(e.g.\ SEGA \cite{sega2024} 0.849, $\omega$-GCN~\cite{burdisso2023}/HCAG~\cite{niu2021}
0.838/0.816, RED~\cite{zhang2025} 0.90) are
...
context that a single backchanneled turn lacks (\Cref{fig:chunk}). This is the
most important design choice in the model: in development ablations, widening
the window from 2 to 4--6 exchanges is what separates an effective model from an
ineffective one.
...
balanced metric in the depression-detection literature. Following that
convention, we use the names \emph{UAR} and \emph{micro-F1} for these two
quantities throughout. Both threshold-dependent metrics are computed at the
...
text-only single-model literature, on the metrics those works report (full
metric set for TC-MIL in \Cref{tab:ladder}). TC-MIL cells are the 30-seed
per-seed mean$\,{\pm}\,$std (top) and the subject-level bootstrap 95\% CI
(bottom, 2000 resamples of the 47 test subjects), matching \Cref{tab:ladder};
baseline cells are as reported by the source. micro-F1 $\equiv$ accuracy;
UAR $=$ unweighted average recall ($\equiv$ balanced accuracy); ``--'' $=$ not
reported.}
\label{tab:external}
\centering
\small
...
interviewer-prompt bias control, or a subject-level interval. The Bi-LSTM with
self-attention of \cite{10006532} reports a slightly higher test macro-F1 of
$0.7834$, but that figure is its dialogue setting that ingests the interviewer's
questions --- exactly the prompt shortcut we control for (\Cref{sec:interpret})
--- and its participant-only setting falls to $0.7235$; like \cite{agarwal2024} it
is a single run with no AUC, micro-F1, or subject-level interval, so it too sits
within one seed standard deviation of our $0.774\pm0.029$. On UAR
(\Cref{tab:external}) the picture is consistent: our $0.77\pm0.03$ exceeds
...
settings); \cite{agarwal2024} reports a higher single-run UAR of $0.83$, but
that point falls inside our subject-level bootstrap interval $[0.59, 0.92]$, so
on 47 test subjects the difference is not resolvable, and it again comes without
an AUC, a micro-F1, a bias control, or an interval. TC-MIL thus matches
the best prior text-only single model on macro-F1, is competitive on UAR, and
additionally supplies a threshold-free AUC (0.864), micro-F1 (0.814), an
explicit bias control (\Cref{sec:interpret}), a per-chunk faithfulness analysis,
and 30-seed plus bootstrap uncertainty. The subject-level bootstrap intervals for the
...
same idea TC-MIL later reuses as a pure auxiliary regulariser. We label the three by their distinguishing
mechanism rather than by their legacy version numbers. \emph{SS-DAMIL-R\,(MH)}
(formerly v9) is the canonical symptom-supervised model with \emph{multi-head}
pooling and is the legacy ceiling; \emph{SS-DAMIL-R\,(GSI)} (v25) replaces the
additive symptom signal with a \emph{gated symptom-injection} block; and
\emph{SS-DAMIL-R\,(Conv)} (v29) prepends a 1-D \emph{convolutional} front-end
...
(dialogue chunks). micro-F1 $\equiv$ accuracy; pos-F1 $=$ positive-class F1;
UAR $=$ unweighted average recall ($\equiv$ balanced accuracy). The
...
\Cref{tab:external}). Only the class weight (lever 3) transfers robustly; the
sweep, the threshold variants, and levers 6--7 are discussed in the text.
...
headline model. The official test --- evaluated
...
(DAMIL-R, MH); the paired significance over the matched (fold, seed) subset is
...
% Two-encoder ensemble extracted to sec_ensemble.tex (single-model scope).
%\input{sec_ensemble.tex}
...
baseline on almost every metric (the few exceptions are MH recall/ROC-AUC,
...
role-aware DAMIL-R/MH/GSI/Conv on the F1 metrics --- at $n=15$ matched runs the
...
($p$-values, $n=15$ matched runs). ``$^{*}$'' denotes $p<0.05$. UAR $=$
unweighted average recall ($\equiv$ balanced accuracy); micro-F1 $\equiv$
accuracy. Monte Carlo (MC) is significant almost everywhere; $K$-Fold is
...
bge-large GRU, 5-seed ensemble, prevalence-matched threshold. Removing the
prompts lowers \emph{every} metric --- the interviewer turns add context, not a
positional shortcut.}
\label{tab:ponly}
\centering
\small
...
honest that its attention is not a faithful per-instance saliency. The source
...
% Cited only by the commented-out ensemble section (sec_ensemble.tex) and the
% removed two-branch/LLM comparison; re-enable with \input{sec_ensemble.tex}.
...
\bibitem{burdisso2023} S. Burdisso, E. Villatoro-Tello, S. Madikeri, and
P. Motlicek, ``Node-weighted graph convolutional network for depression
detection in transcribed clinical interviews,'' in \emph{Proc. Interspeech},
2023.
\bibitem{niu2021} M. Niu, K. Chen, Q. Chen, and L. Yang, ``HCAG: A hierarchical
context-aware graph attention model for depression detection,'' in \emph{Proc.
IEEE Int. Conf. Acoustics, Speech and Signal Processing (ICASSP)}, 2021,
pp. 4235--4239.
\bibitem{zhang2025} L. Zhang, Z. Gao, D. Zhou, and Y. He, ``Explainable
depression detection in clinical interviews with personalized
retrieval-augmented generation,'' in \emph{Findings of the Association for
Computational Linguistics: ACL 2025}, 2025, pp. 9927--9944.
