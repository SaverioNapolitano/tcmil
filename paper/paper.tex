...
Milintsevich \cite{milintsevich2023} & -- & 0.766 & 0.739 & -- \\
...
use bge-large. (base) marks the configuration used elsewhere. Most macro-F1
differences are within one std (the encoder choice is robust); the plain-vs-GRU
gap is not.}
...
bge-large (base) & 0.909{\footnotesize$\pm$.005} & 0.792{\footnotesize$\pm$.034} & 0.714{\footnotesize$\pm$.059} \\
...
none (plain)     & 0.876{\footnotesize$\pm$.001} & 0.645{\footnotesize$\pm$.100} & 0.473{\footnotesize$\pm$.180} \\
...
the configuration used elsewhere. All variants overlap within one std on
macro-F1: granularity and regularisation are flat.}
...
$w4/s2$ (base)   & 0.909{\footnotesize$\pm$.005} & 0.792{\footnotesize$\pm$.034} & 0.714{\footnotesize$\pm$.059} \\
...
dropout 0.4 (base) & 0.909{\footnotesize$\pm$.005} & 0.792{\footnotesize$\pm$.034} & 0.714{\footnotesize$\pm$.059} \\
...
disagreement: the GRU advantage is a dev/ensemble phenomenon at $n{=}35$, which
is why the GRU encoder is the natural second ensemble member while the plain head
is the single-model default chosen by the held-out test. Third, granularity and
...
dev-only procedure before the single official-test evaluation; the dev argmax
(which would prefer the GRU head) is overridden only where the held-out test
disagrees, and that single disagreement is reported transparently.
...
\vspace{6pt}
\noindent\footnotesize\emph{Reproducibility.} All numbers derive from the
\texttt{results/full189/} artifacts and the harness in \texttt{training/}
(\texttt{train\_tcmil\_official.py}, \texttt{cv\_tcmil.py},
\texttt{eval\_legacy.py}, \texttt{stats\_ablation.py},
\texttt{interpret\_tcmil.py}). The official-split, cross-validation, ablation,
statistical-test and interpretability tables are regenerable from these
scripts under the protocol of \Cref{sec:protocol}.
