...
\begin{document}
\raggedbottom  % avoid \flushbottom stretching float separations into mid-column gaps
...
embeds them with a frozen sentence encoder, and aggregates them with
gated-attention pooling --- optionally over a bidirectional-GRU context, though
the order-free (plain) variant is deployed as the headline for its faithful
attribution and robustness under distribution shift --- regularised by an
auxiliary PHQ-8 symptom head. Evaluated strictly on the official AVEC2017 split (per-seed mean
...
cross-seed stable, free of the prompt shortcut, and clinically grounded; the
deployed plain head admits a faithful occlusion-based attribution, whereas the
selective GRU attention is informative in bulk but not a faithful per-instance
explanation.
\end{abstract}
...
near-identical vectors, so pooling cannot reliably recover information that is
absent from the instance representation, however elaborate the aggregator. We observed this ceiling directly across
...
identified, where exploiting the fixed position of the interviewer's
mental-health questions inflates macro-F1 from $0.85$ (patient-only) to $0.90$
(\Cref{sec:related}). A method that is not measured under a
...
text-only baseline on positive-, macro- and micro-F1, and matches the
...
context that a single backchanneled turn lacks (\Cref{fig:chunk}). Within our
ablation study this is the most influential design choice:
...
weighted auxiliary loss, the latter an eight-way multi-label binary
cross-entropy over the binarised PHQ-8 symptom logits (weight $\lambda=0.3$).
The DAIC-WOZ training split is imbalanced
...
\textbf{Metrics.} Writing TP, TN, FP, FN for the per-class confusion counts,
precision is $\mathrm{TP}/(\mathrm{TP}{+}\mathrm{FP})$, recall
$\mathrm{TP}/(\mathrm{TP}{+}\mathrm{FN})$, and F1 their harmonic mean. We report
ROC-AUC (threshold-free ranking), positive-class F1, macro-F1 (the unweighted
both-class mean F1), micro-F1, and the unweighted average recall (UAR). Two
equalities follow from the single-label binary setting and are worth stating
explicitly. \emph{Micro-F1 $=$ accuracy}: micro-averaging pools the confusion
counts across classes, and since each instance carries exactly one prediction
every error is at once one FP and one FN, so $\sum\mathrm{FP}=\sum\mathrm{FN}$,
micro-precision $=$ micro-recall, and hence
$\text{micro-F1}=(\mathrm{TP}{+}\mathrm{TN})/N=\text{accuracy}$.
\emph{UAR $=$ balanced accuracy}:
$\text{UAR}=\tfrac12\!\left(\tfrac{\mathrm{TP}}{\mathrm{TP}+\mathrm{FN}}
+\tfrac{\mathrm{TN}}{\mathrm{TN}+\mathrm{FP}}\right)$ is the mean of the
per-class recalls (sensitivity and specificity), identical to balanced accuracy
by definition, and is the standard balanced metric in the depression-detection
literature. Both threshold-dependent metrics are computed at the same a-priori
prevalence threshold as macro-F1, so they are honest and not separately tuned
oracles.
...
\textbf{Configurations.} The paper reports several analyses that, by design, use
different pooling heads and seed budgets: the headline single-model claims use the
order-free (plain) attention head fixed a priori for faithful attribution, the
interpretability and interviewer-prompt probes use the selective BiGRU head whose
attention is informative, and the cross-corpus transfer uses a participant-only
variant matched to E-DAIC's un-diarized format. \Cref{tab:config} states, for each
results table, which head and seed budget it uses and why.

\begin{table}[t]
\caption{Which configuration each result uses. ``Plain'' is the order-free
attention head (faithful linear attribution); ``GRU'' is the BiGRU temporal head
(selective, informative attention); ``P-only'' strips interviewer turns. Seed
budget: per-seed mean over $N$ seeds unless marked \emph{ens.}\ (probability
ensemble) or \emph{per-run} (fold$\times$seed). Split is the evaluation split;
\emph{CV} is the internal cross-validation pool (train+dev) and \emph{E-DAIC}
the transfer target corpus.}
\label{tab:config}
\centering
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.17\columnwidth}>{\raggedright\arraybackslash}p{0.27\columnwidth}>{\raggedright\arraybackslash}p{0.185\columnwidth}>{\raggedright\arraybackslash}p{0.27\columnwidth}@{}}
\toprule
\textbf{Result} & \textbf{Head} & \textbf{Seeds / split} & \textbf{Purpose} \\
\midrule
\Cref{tab:external} & Plain & 30, test & vs.\ literature \\
\Cref{tab:ladder} & Plain\,/\,per-arch & 30, test & architecture gain \\
\Cref{tab:levers} & Plain & 30, test & decision rule \\
\Cref{tab:abldesign-rep,tab:abldesign-hp} & Plain (GRU cmp.) & 5, dev & config selection \\
\Cref{tab:cv} & GRU & per-run, CV & internal ranking \\
\Cref{tab:interp} & GRU, $pw1$ & 5, test & faithfulness \\
\Cref{fig:faith} & GRU+Plain, $pw1$ & 5, test & faithfulness \\
\Cref{tab:ponly} & GRU, P-only & 5 ens., test & bias control \\
% \Cref{tab:ft-grid}--\ref{tab:ft-cv} & GRU (FT harness) & 5\,/\,30, dev/test & encoder adapt. \\ % FT section is \iffalse'd out; tables tab:ft-* do not exist in the build
\Cref{tab:edaic} & P-only, Plain\,/\,GRU & 30, E-DAIC & zero-shot \\
\bottomrule
\end{tabular}
\end{table}
...
official test split against the verified text-only single-model baselines of
\Cref{sec:related}. For direct comparison the table shows only the metrics the text-only
...
Bi-LSTM with self-attention of \cite{10006532} reports $0.7834$ in a dialogue
setting that, like ours, ingests the interviewer's questions; without a bias
control, however, it cannot be placed in either the clean or the
interviewer-prompt-inflated regime, so it is competitive but \emph{unverified}.
Its participant-only setting is $0.7235$, and its dialogue figure sits within one
seed standard deviation of $0.774\pm0.029$. Both \cite{agarwal2024} and the
Bi-LSTM are single runs without an AUC, a micro-F1, an interviewer-prompt bias
control, or a subject-level interval --- the last being exactly the check
(\Cref{sec:interpret}) that places \emph{our} dialogue number in the
verified-clean regime.
...
ladder (\Cref{tab:ladder}).

\textbf{SS-DAMIL-R} (\emph{Symptom-Supervised
...
(dialogue chunks). ``$^{*}$'' on a TC-MIL cell marks that TC-MIL significantly
exceeds \emph{every} baseline at that metric ($p<0.05$ under both Welch's
$t$-test and the Mann--Whitney $U$ test over the 30 per-seed scores); PR-AUC is
the only metric not significant against all role-aware baselines.}
\label{tab:ladder}
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
...
nothing to select. Second, \emph{context is what matters, and placing it inside the instance is the
more effective and direct route to it}. DAMIL-R reinstates context in the
\emph{aggregator} --- cross-role attention reading each answer in light of its
question --- and that alone is the biggest legacy lever (it adds 0.05--0.11 AUC
over flat MIL), confirming context is the operative variable. TC-MIL obtains the
same context one step earlier, by placing the dialogue inside the instance text,
and the ladder shows the instance route wins: it is both higher in the mean and
roughly five times tighter in seed variance than the best aggregator-route model.
The aggregator and the instance are therefore two ways to supply the same missing
context; the result is not that aggregation is unnecessary but that the instance
is the better place to put the context. Third, the strongest pooled-CV performers all instantiate the
MH architecture and differ only in augmentation hyperparameters, and the two
genuinely distinct variants (GSI, Conv) do not surpass MH.

TC-MIL is not only higher in the mean but also more stable: its AUC standard
deviation is roughly five times tighter than that of the best legacy model.
Against the role-aware models the per-run cross-validation differences are not
always significant (\Cref{sec:internal}); TC-MIL's clearest margins are on the
official split and the $K$-Fold internal split.

\Cref{fig:forest} shows the official-split macro-F1 of the whole ladder with
subject-bootstrap 95\% CIs and the reference baseline \cite{milintsevich2023}:
TC-MIL is the only model whose \emph{mean} clears the 0.739 line --- its
bootstrap interval, like every model's, still contains it.
...
(1.0), $0.754$ (1.5), $0.751$ (2.0) --- so $pos\_weight\!=\!1.0$ is the clear peak and
...
prevalence-threshold operating point sits about 0.06 below the model's own oracle
on the threshold--macro-F1 curve and the reliability curve is skewed;
$pos\_weight=1.0$ moves the operating point onto the peak, closing the gap to
$\approx$0.02 and straightening the reliability curve --- a calibration fix, not
a ranking one, since the AUC barely moves. The symptom-informed decision (lever 4) could
...
plain head also generalises best. Third, $w4/s2$ matches the larger $w6/s3$, and the regularisation settings move
the dev \emph{AUC} negligibly ($0.908$--$0.914$). They do shift the thresholded
dev metrics more --- dropout 0.3/0.5 and aux-weight 0.0/0.5 edge the base on
macro- and positive-class F1 by $\sim$0.02--0.05 --- but every such gap is within
one seed std at $n{=}5$ and the $n{=}35$ dev thresholded metrics are quantised,
so we treat them as noise rather than chase the dev argmax (the same
pre-registration stance applied to the head above). The base ($w4/s2$,
dropout 0.4) is therefore kept as a sensible default. The auxiliary loss, though
droppable at no dev-AUC cost (and a slightly higher dev F1), is retained for the
clinical read-out (\Cref{sec:interpret}). Crucially, the configuration was frozen by this pre-registered,
...
The official-split comparison is external. To check
...
(5 repeats $\times$ 5 folds, 5 seeds per fold) and Monte Carlo (repeated stratified shuffle) splits, with the
...
scale-safe across folds and uses no labels. Because this comparison probes the
architecture \emph{ranking} rather than per-chunk faithfulness, TC-MIL is
evaluated here with its GRU temporal head (the development-best configuration);
the order-free plain head fixed for the faithful headline (\Cref{sec:interpret})
is not required for a ranking check, and the official-split tables retain it.
...
repeats$\times$folds$\times$seeds). ``$^{*}$'' on a TC-MIL cell marks that
TC-MIL significantly exceeds \emph{every} baseline at that metric and protocol
($p<0.05$ under both the Wilcoxon signed-rank and paired $t$-tests over the
matched runs; $n=125$ K-Fold, $n=50$ MC). Under Monte Carlo only the
threshold-free AUCs clear this bar: the thresholded-metric gaps stay significant
against the flat baselines (and most SS-DAMIL-R variants) but tie against the
strongest baseline, DAMIL-R.}
\label{tab:cv}
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
...
\midrule
\multirow{8}{*}{$K$-Fold}
 & dialogue mean & \celld{.53}{.07}{.52,.55} & \celld{.13}{.24}{.09,.17} & \celld{.14}{.25}{.09,.18} & \celld{.13}{.22}{.09,.17} & \celld{.47}{.10}{.45,.49} & \celld{.70}{.05}{.69,.71} & \celld{.56}{.13}{.54,.58} & \celld{.69}{.11}{.68,.71} \\
\addlinespace[1.5pt]
 & flat MIL, mean & \celld{.52}{.06}{.51,.53} & \celld{.13}{.26}{.08,.17} & \celld{.08}{.18}{.05,.11} & \celld{.08}{.18}{.05,.12} & \celld{.45}{.08}{.44,.47} & \celld{.70}{.03}{.70,.71} & \celld{.48}{.12}{.46,.50} & \celld{.63}{.12}{.61,.65} \\
\addlinespace[1.5pt]
 & flat MIL, attn & \celld{.53}{.07}{.52,.54} & \celld{.17}{.29}{.11,.22} & \celld{.12}{.22}{.08,.16} & \celld{.12}{.21}{.09,.16} & \celld{.47}{.10}{.45,.49} & \celld{.70}{.05}{.69,.71} & \celld{.51}{.13}{.49,.53} & \celld{.64}{.11}{.62,.66} \\
\addlinespace[1.5pt]
 & DAMIL-R & \celld{.67}{.12}{.65,.68} & \celld{.53}{.16}{.50,.56} & \celld{.53}{.17}{.50,.56} & \celld{.53}{.17}{.50,.56} & \celld{.66}{.12}{.64,.68} & \celld{.72}{.10}{.70,.74} & \celld{.60}{.15}{.57,.63} & \celld{.73}{.13}{.71,.75} \\
\addlinespace[1.5pt]
 & SS-DAMIL-R (MH)& \celld{.62}{.11}{.60,.64} & \celld{.46}{.21}{.43,.50} & \celld{.44}{.22}{.40,.48} & \celld{.44}{.20}{.40,.47} & \celld{.61}{.12}{.59,.63} & \celld{.70}{.09}{.68,.71} & \celld{.55}{.14}{.52,.57} & \celld{.69}{.13}{.67,.71} \\
\addlinespace[1.5pt]
 & SS-DAMIL-R (GSI)& \celld{.62}{.12}{.60,.64} & \celld{.46}{.21}{.42,.50} & \celld{.44}{.22}{.40,.48} & \celld{.44}{.21}{.40,.48} & \celld{.61}{.13}{.59,.64} & \celld{.69}{.09}{.68,.71} & \celld{.54}{.15}{.51,.57} & \celld{.69}{.13}{.67,.72} \\
\addlinespace[1.5pt]
 & SS-DAMIL-R (Conv)& \celld{.63}{.11}{.61,.65} & \celld{.49}{.19}{.45,.52} & \celld{.45}{.21}{.42,.49} & \celld{.45}{.19}{.42,.49} & \celld{.62}{.12}{.60,.64} & \celld{.70}{.09}{.69,.72} & \celld{.55}{.15}{.52,.57} & \celld{.69}{.13}{.67,.71} \\
\addlinespace[1.5pt]
...
\midrule
\multirow{8}{*}{MC}
 & dialogue mean & \celld{.53}{.06}{.51,.55} & \celld{.15}{.26}{.08,.23} & \celld{.11}{.23}{.05,.18} & \celld{.11}{.20}{.06,.17} & \celld{.46}{.09}{.43,.48} & \celld{.68}{.03}{.68,.69} & \celld{.57}{.12}{.54,.60} & \celld{.71}{.09}{.68,.73} \\
\addlinespace[1.5pt]
 & flat MIL, mean & \celld{.52}{.05}{.51,.53} & \celld{.12}{.26}{.06,.20} & \celld{.08}{.19}{.04,.14} & \celld{.09}{.18}{.04,.14} & \celld{.45}{.08}{.43,.47} & \celld{.68}{.03}{.68,.69} & \celld{.52}{.11}{.49,.55} & \celld{.63}{.09}{.61,.66} \\
\addlinespace[1.5pt]
 & flat MIL, attn & \celld{.53}{.06}{.51,.54} & \celld{.11}{.23}{.06,.18} & \celld{.11}{.22}{.05,.17} & \celld{.10}{.20}{.05,.16} & \celld{.45}{.09}{.43,.48} & \celld{.68}{.04}{.67,.69} & \celld{.52}{.11}{.49,.55} & \celld{.63}{.10}{.61,.66} \\
\addlinespace[1.5pt]
 & DAMIL-R & \celld{.71}{.10}{.68,.74} & \celld{.61}{.15}{.56,.65} & \celld{.59}{.15}{.54,.63} & \celld{.60}{.15}{.55,.64} & \celld{.71}{.10}{.68,.74} & \celld{.75}{.08}{.73,.78} & \celld{.63}{.17}{.58,.68} & \celld{.76}{.11}{.73,.79} \\
\addlinespace[1.5pt]
 & SS-DAMIL-R (MH)& \celld{.65}{.12}{.62,.68} & \celld{.50}{.19}{.45,.55} & \celld{.51}{.22}{.45,.57} & \celld{.50}{.20}{.44,.55} & \celld{.64}{.13}{.61,.68} & \celld{.70}{.10}{.68,.73} & \celld{.59}{.16}{.55,.63} & \celld{.72}{.12}{.69,.75} \\
\addlinespace[1.5pt]
 & SS-DAMIL-R (GSI)& \celld{.66}{.10}{.64,.69} & \celld{.53}{.17}{.48,.57} & \celld{.53}{.19}{.47,.58} & \celld{.52}{.17}{.47,.56} & \celld{.66}{.11}{.63,.68} & \celld{.72}{.08}{.69,.74} & \celld{.57}{.15}{.53,.62} & \celld{.71}{.09}{.68,.73} \\
\addlinespace[1.5pt]
 & SS-DAMIL-R (Conv)& \celld{.62}{.12}{.59,.66} & \celld{.48}{.21}{.42,.54} & \celld{.44}{.23}{.37,.50} & \celld{.44}{.21}{.38,.50} & \celld{.61}{.13}{.58,.65} & \celld{.69}{.10}{.67,.72} & \celld{.56}{.15}{.52,.60} & \celld{.69}{.12}{.66,.72} \\
\addlinespace[1.5pt]
...
TC-MIL has the highest mean in every column of both protocols (\Cref{tab:cv}).
The per-run score distributions overlap with the role-aware DAMIL-R and MH
(per-run std $\approx0.12$), but the matched-pair tests --- whose significance is
marked by the asterisks in \Cref{tab:cv} --- resolve the means: TC-MIL
significantly exceeds every baseline on the primary metrics under $K$-Fold and on
the threshold-free ranking throughout, the strongest baseline (DAMIL-R) closing
the thresholded gap only under Monte Carlo's larger training splits. The pattern
matches the ladder: TC-MIL leads every protocol \emph{headline}, with its
clearest thresholded margins on the official split and the $K$-Fold internal
split.

\section{Interpretability}
\label{sec:interpret}

We treat interpretability as a measured result.
...
official train split; the headline faithfulness, stability and bias metrics are
reported once on test, and ablation on dev). TC-MIL exposes three interpretable surfaces: gated attention over
...
attended chunks are removed), sufficiency@$k$ (the drop when only the top-$k$ are
kept), and the Spearman correlation between each chunk's attention and its
leave-one-out $\Delta$probability.

\Cref{tab:interp} reports the aggregate metrics. Sufficiency@$k$ is negative for
both models: the top-$k$ attended chunks alone already drive the probability to
or above the full-bag value, so the salient chunks suffice, and together with the
positive comprehensiveness this shows the top chunks carry the bulk of the
decision even where the per-chunk attention is not faithful. We then probe
per-chunk faithfulness directly: for every dialogue chunk we record its attention weight
...
stability is correspondingly high (0.97).

Adding the GRU temporal head couples
...
\emph{negative} (per-interview mean $-0.31\pm0.32$, $n=46$, one-sample $t$ vs.\ 0
$p=5.7\!\times\!10^{-8}$; the rank-pooled slope of \Cref{fig:faith} is
$-0.28$). Yet the GRU model's \emph{aggregate} comprehensiveness
...
\begin{table}[t]
\caption{Interpretability on the \emph{test} split, 5-seed,
$pos\_weight\!=\!1.0$ (GRU head). Dial.\ vs.\
...
is tested against 0 by a one-sample $t$-test ($^{*}$ $p<0.05$). The remaining
rows are 5-seed aggregate point estimates, so no test is attached.
Interviewer-kw attn.\ mass is the fraction of total attention placed on chunks
containing the interviewer's mental-health-question keywords.
$^{\dagger}$Symptom AUC is the dev split: the test CSV lacks PHQ-8 item labels.}
\label{tab:interp}
\centering
\small
\setlength{\tabcolsep}{4pt}
...
\midrule
Attention stability (Spearman) & $0.60${\tiny$\pm$.12} & $0.66${\tiny$\pm$.15} \\
Attn$\leftrightarrow$occlusion corr. & $-0.31${\tiny$\pm$.32}$^{*}$ & $-0.01${\tiny$\pm$.42} \\
Comprehensiveness @1/@5 & .09/.27 & .08/.25 \\
Sufficiency @1/@5 & $-.22$/$-.10$ & $-.25$/$-.17$ \\
Position--attention corr. & $-0.04$ & $+0.19$ \\
Interviewer-kw attn.\ mass & 0.67 & 0.00 \\
Symptom AUC (8-item range)$^{\dagger}$ & .74--.94 & .75--.91 \\
\bottomrule
\end{tabular}
\end{table}
...
$\approx$0 ($-0.03$ dev, $-0.04$ test): attention is spread across the
...
\Cref{tab:ponly}), so the interviewer turns do contribute signal --- but
degradation alone cannot say whether that signal is a shortcut or legitimate
context, since removing either would hurt. What settles it is the nature of the
prompts: because DAIC's are semi-structured --- the same PHQ-style questions, in
the same order, for every subject --- they are near-constant across classes and
so cannot by themselves discriminate the label. The signal they add can
therefore only contextualise the participant's own answers, not act as a
class-discriminative shortcut. Stripped of that context the model leans
\emph{more} on position (its position--attention correlation rises from $-0.04$
to $+0.19$, \Cref{tab:interp}) --- the opposite of what a prompt-shortcut model
would show, which relies on the late mental-health-question region while the
prompts are present and collapses once they are removed. Here position-reliance
appears only \emph{after} the prompts are gone, confirming the deployed model
used them as context rather than as a positional shortcut. The surviving signal is answer-driven: with the prompts gone the
...
participant-only accuracy gain.
...
The PHQ-8 symptom head is a genuine clinical signal:
...
\end{axis}
\end{tikzpicture}
\caption{Per-item PHQ-8 symptom-head ROC-AUC (GRU head, dev, 5-seed). Every symptom is
...
relative to the auto-$pos\_weight$ model (dev stability 0.77, occlusion
correlation $-0.23$), the headline $pos\_weight\!=\!1.0$ model (\Cref{tab:interp})
is \emph{less} stable (dev 0.60) and \emph{less} per-chunk faithful (dev
occlusion correlation $-0.32$), while the no-shortcut result is unchanged. The recall-biased loss apparently produced
peakier, more selectable attention; de-biasing improves the decision but
slightly blurs the per-chunk attribution.
...
a different ASR (automatic speech recognition) front end, a different recording protocol. We test this directly
...
DAIC-WOZ, and is reported here as a post-hoc probe of whether the
development-selected model also transfers. Neither configuration's E-DAIC test score was used to
...
We report the threshold-free ROC-AUC and PR-AUC as the primary transfer metrics: a decision threshold cannot be tuned on the target without
leaking it, and the source threshold is mis-calibrated under domain shift.
...
chance and consistent with the cross-ASR, cross-cohort gap. The thresholded operating point degrades more
...
\textbf{(2) The development-best configuration generalizes worst --- in-domain
and across corpora.} Adding a BiGRU temporal-context layer is the strongest
source model \emph{only on the DAIC-WOZ development split} (per-seed dev AUC
$0.900$ vs.\ $0.857$ for order-free pooling). That edge survives to \emph{no}
held-out test set: the GRU is lower on the in-domain DAIC test ($0.786$ vs.\
$0.819$) and lower again zero-shot on E-DAIC ($0.662$ vs.\ $0.716$; the $0.052$
gap is significant under a seed-paired Wilcoxon signed-rank test over the 30
seeds, $p<10^{-4}$). The primary lesson is therefore one of \emph{model
selection}: development AUC is an unreliable signal here --- the temporal layer
overfits the split it is tuned on rather than encoding transferable structure,
so the dev-best model is the wrong choice on every test set, in-domain test
included. The cross-corpus result sharpens this: the gap widens out-of-corpus,
and the permutation-invariant order-free pooler is the more robust inductive
bias under domain shift, making it both the safer in-domain choice and the
better generalizer.
...
shown here because E-DAIC is the new (transfer) result --- the in-domain
DAIC-WOZ bootstrap CIs are already in \Cref{tab:external,tab:ladder}.
...
artefacts. The transfer also sharpens a model-selection lesson: the head that
tops the \emph{development} split (BiGRU) is worse on every held-out test set ---
the in-domain DAIC test and, by a wider and significant margin, zero-shot
E-DAIC --- so development AUC is an unreliable selection signal, and the
order-free pooler is the more robust inductive bias under domain shift.

\section{Discussion and Limitations}

The gain is attributable to the dialogue-chunk representation and to one
calibration-level change (the class weight): context is the operative variable,
and placing it inside the instance beats supplying it in the aggregator, on both
mean and stability.

Several limitations bound the claim. \textbf{Sample size.} With 47 test subjects
(14 positive) the bootstrap intervals are wide; our seed-level significance
concerns the architecture's expected score, not a subject-level generalisation
guarantee. \textbf{A prevalence prior.} The a-priori threshold is leakage-free of
test \emph{labels} but assumes deployment prevalence equals training prevalence
($\approx$0.28), so the headline F1 --- not the AUC --- depends on that prior
(\Cref{tab:levers}). \textbf{Attention is not a faithful per-chunk explanation};
the faithful signal lives in the occlusion attribution, not a heatmap.
\textbf{Single corpus and modality.} Selection is on one corpus, text only, and
the E-DAIC result is zero-shot and single-shot. \textbf{Label provenance.} The
binary label derives from PHQ-8, the instrument the auxiliary head predicts, so
the read-out explains a PHQ-8-grounded label, not an independent diagnosis.
\iffalse
\textbf{Ethical considerations.} TC-MIL is a research screen, not a diagnostic
instrument. At a test AUC of $0.86$ in-domain and $\approx\!0.72$ zero-shot, with
F1 contingent on a prevalence assumption, it is far from the reliability a clinical
decision would require, and the wide subject-level intervals mean an individual
prediction is uncertain. The model operates on transcripts of clinical interviews,
which remain highly sensitive even after de-identification, and the
interpretability analysis shows the decision leans on participants' own symptom
disclosures --- so any deployment must treat both the input and the output as
protected health information. Two failure modes carry asymmetric cost in a
screening setting: a false negative may withhold care, a false positive may
stigmatise; the prevalence-matched operating point trades these off at the
training prevalence, which may not match a target population. Finally,
generalisation across cohorts, ASR front-ends and demographics is untested beyond
the single zero-shot E-DAIC transfer, so the model should not be applied to a new
population without re-validation and human oversight. We release the model for
reproducible research only and make no clinical-deployment claim.
\fi
\section{Conclusion}
...
% Appendix with the statistical-test tables (Tab. VIII/IX) commented out
% (kept in appendix.tex). To re-enable: uncomment the two lines below and
% restore the \Cref pointers in the Statistical Significance Summary
% (tab:stats, tab:cvstats, sec:appendix-stats).
%\appendices
%\input{appendix}
