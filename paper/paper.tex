...
ensemble) or \emph{per-run} (fold$\times$seed).}
\label{tab:config}
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{@{}p{0.30\columnwidth}p{0.16\columnwidth}p{0.18\columnwidth}p{0.22\columnwidth}@{}}
\toprule
\textbf{Result} & \textbf{Head} & \textbf{Seeds} & \textbf{Purpose} \\
\midrule
\Cref{tab:external} (headline) & Plain & 30 & vs.\ literature \\
\Cref{tab:ladder} (ladder) & Plain\,/\,per-arch & 30 & architecture gain \\
\Cref{tab:levers} (levers) & Plain & 30 & decision rule \\
\Cref{tab:abldesign-rep,tab:abldesign-hp} & Plain (GRU cmp.) & 5 dev & config selection \\
\Cref{tab:cv} (internal CV) & GRU & per-run & internal ranking \\
\Cref{tab:interp} (interp.) & GRU & 5 dev/test & faithfulness \\
\Cref{fig:faith} (faith.) & GRU\,+\,Plain & 5 dev/test & faithfulness \\
\Cref{tab:ponly} (prompt ctrl) & GRU, P-only & 5 ens. & bias control \\
% \Cref{tab:ft-grid}--\ref{tab:ft-cv} (FT) & GRU (FT harness) & 5\,/\,30 & encoder adapt. \\ % FT section is \iffalse'd out; tables tab:ft-* do not exist in the build
\Cref{tab:edaic} (transfer) & P-only, Plain\,/\,GRU & 30 & zero-shot E-DAIC \\
\bottomrule
\end{tabular}
\end{table}
...
(5 repeats $\times$ 5 folds, 5 seeds per fold $=125$ per-run evaluations per
model) and Monte Carlo (repeated stratified shuffle) splits, with the
...
\begin{table}[t]
\caption{Interpretability, 5-seed (GRU head). Dial.\ vs.\
...
to $+0.32$, \Cref{tab:interp}) --- the opposite of what a prompt-shortcut model
...
We report the threshold-free ROC-AUC as the primary transfer metric (with PR-AUC
quoted in the text): a decision threshold cannot be tuned on the target without
