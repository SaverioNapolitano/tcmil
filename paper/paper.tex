...
\Cref{tab:cv} (internal CV) & Plain & per-run & internal ranking \\
\Cref{tab:interp},~\Cref{fig:faith} & GRU\,+\,Plain & 5 dev/test & faithfulness \\
\Cref{tab:ponly} (prompt ctrl) & GRU, P-only & 5 ens. & bias control \\
\Cref{tab:ft-grid}--\ref{tab:ft-cv} (FT) & GRU (FT harness) & 5\,/\,30 & encoder adapt. \\
\Cref{tab:edaic} (transfer) & P-only, none\,/\,GRU & 30 & zero-shot E-DAIC \\
\bottomrule
\end{tabular}
\end{table}
...
(\Cref{tab:stats}), and on the Monte Carlo headline.
...
TC-MIL is the only model whose interval clears the 0.739 line, while the wide
...
$[\,{\approx}0.75,0.80\,]$ band (\Cref{tab:levers}). \textbf{Configuration
heterogeneity.} Different analyses use different heads and seed budgets (the
order-free plain head for the headline, the GRU for the interpretability and
bias-control probes, a participant-only variant for transfer); \Cref{tab:config}
maps each, but the reader must track which model a given number describes.
