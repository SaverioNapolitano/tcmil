...
regime identified by \cite{burdisso2024}.
...
\textbf{Other test-set numbers.} To the best of our knowledge, only a few papers in the literature are both
...
(\Cref{sec:interpret}), a per-chunk faithfulness analysis, and 30-seed plus
...
\emph{ranking} requires fine-tuning the encoder. The
final recipe is therefore the base model with $pos\_weight=1.0$; the remaining
axis that could raise the AUC ceiling is encoder fine-tuning.
...
(\Cref{sec:interpret}), which is a core requirement of our single-model claim.
...
\cite{burdisso2024} identify as the shortcut. This is corroborated at the
...
close the gap to the two-branch fusion frontier.
...
two-encoder ensemble raises macro-F1 to 0.833. The source code and the trained network weights are
