"""SS-DAMIL-R v25: Poly1-Focal Loss + Gated Attention Pooling + Attention Dropout.

Reverts the training schedule back to v9d (80 epochs, patience 15) to fix the
high variance in v24, while upgrading the architecture with proven MIL
regularisation techniques:
  - Gated Attention Pooling (Ilse et al.)
  - Attention Dropout (Stochastic Instance Masking)
  - Poly1-Focal Loss
"""
...
from models.ss_damil_r import SSDamilRClassifierV25
...
class Poly1FocalLoss(nn.Module):
    """Poly1-Focal Loss for binary classification.
    
    Based on: PolyLoss: A Polynomial Expansion Perspective of Classification Loss
    Functions (ICLR 2022).
    """
    def __init__(self, alpha: float = 0.5, gamma: float = 2.0, epsilon: float = 1.0, smoothing: float = 0.0):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, inputs: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        poly1_term = self.epsilon * (1.0 - pt)
        
        loss = focal_term * bce_loss + poly1_term
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v25: Gated Attention + Poly1-Focal")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v25")
    ...
    # Training Config (reverted to v9d stability)
    ...
    parser.add_argument("--loss_type", type=str, default="poly1_focal")
    parser.add_argument("--label_smoothing", type=float, default=0.05)
    parser.add_argument("--poly1_epsilon", type=float, default=1.0)
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        if args.loss_type == "poly1_focal":
            main_loss_fn = Poly1FocalLoss(
                alpha=alpha, 
                gamma=2.0, 
                epsilon=args.poly1_epsilon, 
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v25 - Gated Attention & Poly1-Focal Loss)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v25 (Gated Attention + Poly1-Focal) CV Results\n\n{report}")
    logger.info("\n" + report)
