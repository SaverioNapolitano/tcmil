"""SS-DAMIL-R v10c: Curriculum Mixup (ablation).

Replaces the fixed-alpha manifold mixup in v9d with a curriculum schedule:
  - No mixup during warmup (epochs 1-5): clean representation learning
  - Progressive alpha: 0.1 → 0.4 over training (mild → strong interpolation)
  - Same-class mixing in early phases, then cross-class mixing later
  - All other v9d components retained: dynamic aux weight, SWA,
    cosine+warmup scheduler, label smoothing
"""
...
# ---------------------------------------------------------------------------
# Curriculum Mixup Scheduler
# ---------------------------------------------------------------------------

class CurriculumMixup:
    """Progressive mixup scheduling for manifold mixup.

    Phases:
    1. No mixup during warmup: let the model learn clean representations
    2. Same-class mixing with mild alpha: safe interpolation
    3. Full cross-class mixing with increasing alpha: strong regularization

    Args:
        warmup_epochs: No mixup during these initial epochs.
        alpha_start: Initial Beta distribution parameter (mild mixing).
        alpha_end: Final Beta distribution parameter (strong mixing).
        schedule_epochs: Number of epochs to ramp from start to end alpha.
        cross_class_epoch: Epoch at which to start mixing across classes.
    """
    ...
    def __init__(
        ...
        alpha_end: float = 0.4,
        schedule_epochs: int = 40,
        cross_class_epoch: int = 20,
    ):
        self.warmup_epochs = warmup_epochs
        self.alpha_start = alpha_start
        self.alpha_end = alpha_end
        self.schedule_epochs = schedule_epochs
        self.cross_class_epoch = cross_class_epoch

    def get_params(self, epoch: int) -> tuple[float | None, bool]:
        """Get mixup parameters for the current epoch.

        Returns:
            (alpha, allow_cross_class): alpha=None means no mixup.
        """
        if epoch <= self.warmup_epochs:
            return None, False

        # Progressive alpha ramp
        progress = min(1.0, (epoch - self.warmup_epochs) / max(1, self.schedule_epochs))
        alpha = self.alpha_start + progress * (self.alpha_end - self.alpha_start)

        # Cross-class mixing enabled after cross_class_epoch
        allow_cross_class = epoch >= self.cross_class_epoch

        return alpha, allow_cross_class


# ---------------------------------------------------------------------------
# Training with Curriculum Mixup
# ---------------------------------------------------------------------------

def train_epoch_v10c(
    model, loader, criterion, optimizer, device,
    max_grad_norm=1.0, noise_std=0.0,
    mixup_alpha=None, allow_cross_class=True,
):
    """Training loop with curriculum manifold mixup.

    When allow_cross_class is False, mixup only occurs between samples
    of the same class. This prevents the model from learning to "average away"
    the discriminative signal before representations are mature.
    """
    model.train()
    ...
    for batch in loader:
        ...
        # Step 3: Curriculum Manifold Mixup
        do_mixup = (
            mixup_alpha is not None
            and mixup_alpha > 0
            and pooled_batch.size(0) >= 2
        )
        ...
        if do_mixup:
            ...
            if allow_cross_class:
                # Full cross-class mixing (same as v9d)
                perm = torch.randperm(pooled_batch.size(0), device=device)
            else:
                # Same-class mixing: shuffle within each class group
                ...
                if len(pos_idx) >= 2:
                    shuffled_pos = pos_idx[torch.randperm(len(pos_idx)).numpy()]
                    for orig, shuf in zip(pos_idx, shuffled_pos):
                        perm[orig] = shuf
                if len(neg_idx) >= 2:
                    shuffled_neg = neg_idx[torch.randperm(len(neg_idx)).numpy()]
                    for orig, shuf in zip(neg_idx, shuffled_neg):
                        perm[orig] = shuf
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v10c: Curriculum Mixup")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v10c")
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v10c - Curriculum Mixup)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v10c (Curriculum Mixup) CV Results\n\n{report}")
    logger.info("\n" + report)
