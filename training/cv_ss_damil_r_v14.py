"""SS-DAMIL-R v14: Dialogue-Aware Depression Detection.

Two key innovations over v9d:
  1. Dialogue-pair preprocessing from raw data: Q-A pairs give the encoder
     context about what question each participant response addresses.
  2. Upgraded encoder: BAAI/bge-large-en-v1.5 (1024-dim) for richer semantic
     representations vs all-mpnet-base-v2 (768-dim).

The model architecture and training strategy are deliberately kept very close
to the proven v9d configuration:
  - Manifold Mixup at the pooled representation level
  - Dynamic auxiliary weight scheduling (0.5 → 0.1)
  - Label Smoothing Focal Loss
  - Cosine Annealing with Warm Restarts + Linear Warmup
  - Stochastic Weight Averaging (SWA)
  - Instance dropout, noise injection

Minor hyperparameter tuning:
  - proj_dim: 64 → 128 (richer encoder features need more capacity)
  - mixup_alpha: 0.2 → 0.3 (more aggressive augmentation)
  - max_epochs: 80 → 100 (more training time for higher-dim features)
  - patience: 15 → 20 (more patience)
  - weight_decay: 1e-4 → 5e-4 (stronger regularization for larger model)
  - max_len: 128 → 256 (Q-A pairs are longer than individual utterances)
"""
...
from dataset import (
    load_all_interviews_dialogue_pairs,
    DualRoleBagDataset,
    collate_dual_role_bags,
)
from models.ss_damil_r import SSDamilRClassifierV14
...
def train_epoch_v14(
    ...
    """Training loop with manifold mixup at the pooled representation level.

    Identical to train_epoch_v9d:
    1. Run the backbone (projector → cross-attn → fusion → pooling) for each sample
    2. Mix the pooled representations (manifold space) with Beta-sampled λ
    3. Run classification heads on the mixed representations
    4. Compute loss on the mixed outputs with mixed targets
    """
    model.train()
    ...
    for batch in loader:
        ...
        if mixup_alpha > 0 and pooled_batch.size(0) >= 2:
            ...
            # Mix in the manifold (pooled representation space)
            mixed_pooled = lam * pooled_batch + (1 - lam) * pooled_batch[perm]

            # Run heads on mixed representations
        ...
        # Track metrics on original (unmixed) predictions
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v14: Dialogue-Aware Depression Detection")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v14")
    ...
    # Model Config (v14)
    parser.add_argument("--encoder_name", type=str, default="BAAI/bge-large-en-v1.5")
    parser.add_argument("--max_len", type=int, default=256)    # Longer for Q-A pairs
    parser.add_argument("--proj_dim", type=int, default=128)   # Larger for richer features
    parser.add_argument("--noise_std", type=float, default=0.05)
    parser.add_argument("--n_pool_heads", type=int, default=2)

    # Training Config (tuned from v9d)
    ...
    parser.add_argument("--mixup_alpha", type=float, default=0.3)   # More aggressive
    parser.add_argument("--batch_size", type=int, default=8)
    parser.add_argument("--max_epochs", type=int, default=100)       # More training time
    parser.add_argument("--lr", type=float, default=1e-4)
    parser.add_argument("--weight_decay", type=float, default=5e-4)  # Stronger regularization
    parser.add_argument("--patience", type=int, default=20)          # More patience
    ...
    # Embedding batch size (for precomputing Q-A pair embeddings)
    ...
    # --- Data Loading (V14: Dialogue Pairs from Raw Data) ---
    logger.info("Loading all interviews with dialogue pairs from raw transcripts...")
    all_interviews = load_all_interviews_dialogue_pairs(args.data_dir)

    # Log some stats about the dialogue pairs
    n_qa_pairs = [len(iv["qa_pairs"]) for iv in all_interviews]
    n_interviewer = [len(iv["interviewer_utterances"]) for iv in all_interviews]
    logger.info(
        f"Q-A pairs per interview: min={min(n_qa_pairs)}, max={max(n_qa_pairs)}, "
        f"mean={np.mean(n_qa_pairs):.1f}, median={np.median(n_qa_pairs):.1f}"
    )
    logger.info(
        f"Interviewer utterances per interview: min={min(n_interviewer)}, max={max(n_interviewer)}, "
        f"mean={np.mean(n_interviewer):.1f}"
    )

    # --- Encoder (V14: bge-large-en-v1.5) ---
    ...
    logger.info(f"Encoder embedding dim: {embedding_dim}")

    # --- Pre-compute Dialogue Pair Embeddings ---
    logger.info("Pre-computing dialogue pair embeddings...")
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        # Dynamic Aux Weight + Diversity Loss (same as v9d)
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, _ = train_epoch_v14(
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v14 - Dialogue-Aware)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v14 (Dialogue-Aware) CV Results\n\n{report}")
    logger.info("\n" + report)
