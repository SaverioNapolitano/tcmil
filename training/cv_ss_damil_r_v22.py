"""SS-DAMIL-R v22: Manifold Mixup + Simplified Baseline Architecture.

Combines the highly robust SSDamilRClassifierV8_2 architecture (from v9a)
with Manifold Mixup applied at the pooled representation level (from v9d).
Removes multi-head pooling and symptom gating in favor of a simpler, less
prone to overfitting baseline, while maintaining the strong F1 boost of
manifold mixup.
"""
...
from models.ss_damil_r_v22 import SSDamilRClassifierV22
...
# ---------------------------------------------------------------------------
# Training with Manifold Mixup (Simplified for single-head)
# ---------------------------------------------------------------------------

class SimplifiedMultiTaskLoss(nn.Module):
    """Multi-task loss without diversity loss tracking."""
    def __init__(self, main_loss_fn, symptom_loss_fn, aux_weight=0.3):
        super().__init__()
        self.main_loss_fn = main_loss_fn
        self.symptom_loss_fn = symptom_loss_fn
        self.aux_weight = aux_weight

    def forward(self, logits, targets, sym_logits, sym_targets, has_symptoms):
        main_loss = self.main_loss_fn(logits, targets)
        sym_loss = self.symptom_loss_fn(sym_logits, sym_targets, has_symptoms)
        total_loss = main_loss + self.aux_weight * sym_loss
        return total_loss, main_loss, sym_loss
...
def train_epoch_v22(
    ...
    model.train()
    total_loss, total_m_loss, total_a_loss = 0, 0, 0
    ...
    for batch in loader:
        ...
        pooled_batch, _ = model.forward_batch_split(
    ...
    return (
        total_loss / n_batches, total_m_loss / n_batches, total_a_loss / n_batches,
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v22: Manifold Mixup + V8.2 Baseline")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v22")
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        # Simplified Multi-Task Criterion (no dynamic weighting needed for simple baseline)
        criterion = SimplifiedMultiTaskLoss(
        ...
        # Dummy wrapper for evaluate_v9 which expects diversity loss
        class CriterionWrapper:
            def __init__(self, crit): self.crit = crit
            def __call__(self, l, t, sl, st, hs, diversity_loss=None):
                return self.crit(l, t, sl, st, hs) + (torch.tensor(0.0),)

        wrapped_criterion = CriterionWrapper(criterion)

        for epoch in range(1, args.max_epochs + 1):
            avg_l, ml, al, _ = train_epoch_v22(
            ...
            v_l, v_ml, v_metrics, v_preds = evaluate_v9(model, val_loader, wrapped_criterion, device)
        ...
        if len(swa) >= 3:
            ...
            _, _, best_val_metrics, _ = evaluate_v9(model, val_loader, wrapped_criterion, device)
            best_val_score = best_val_metrics.get("roc_auc", 0.0)

            swa_model = _create_model()
            swa.apply(swa_model)
            _, _, swa_val_metrics, _ = evaluate_v9(swa_model, val_loader, wrapped_criterion, device)
        ...
        _, _, v_m, v_p = evaluate_v9(model, val_loader, wrapped_criterion, device)
        best_t = find_best_threshold(
            np.array(v_p["true_label"]), np.array(v_p["probability"]),
            metric="loss", pos_weight=neg_to_pos,
        )

        _, _, test_metrics, test_results = evaluate_v9(model, test_loader, wrapped_criterion, device, threshold=best_t)
        checkpoint_path.unlink(missing_ok=True)
        return {**test_metrics, **test_results}

    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v22 - Manifold Mixup + Simple Baseline)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v22 (Manifold Mixup + V8.2 Baseline) CV Results\n\n{report}")
    logger.info("\n" + report)
