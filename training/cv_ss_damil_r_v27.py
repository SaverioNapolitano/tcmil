"""SS-DAMIL-R v27: Linguistic Fusion + V9 Core.

Fuses explicitly extracted clinical linguistic markers (e.g., past-tense rumination, 
negation, disfluencies) directly into the patient embeddings before cross-attention.
This provides the model with hard-to-miss clinical indicators, bypassing the need
to implicitly learn them from 768-D semantic space on a tiny dataset.
"""
...
from models.ss_damil_r import SSDamilRClassifierV27
from extract_linguistic_features import extract_all_ling_features, normalize_ling_features
...
def train_epoch_v27(
    ...
    Instead of mixing logits (v9c), we:
    ...
    3. Run classification heads on both original AND mixed representations
    ...
    for batch in loader:
        ...
        pooled_batch, diversity_loss = model.forward_batch_split(
            ...
            batch["patient_ling_features"].to(device),
            batch["patient_sizes"],
...
def evaluate_v27(model, loader, criterion, device, threshold=0.5):
    """Evaluation for v27 with linguistic features."""
    model.eval()
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            sym_logits = output["symptom_logits"]
            dl = output["diversity_loss"]

            loss, ml, _, _ = criterion(
                logits, target, sym_logits, sym_target, has_sym,
                diversity_loss=dl
            )
    ...
    results = {"probability": all_probs, "true_label": all_labels}

    return total_loss / n_batches, total_ml / n_batches, metrics, results

def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v27: Linguistic Fusion")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v27")
    ...
    def _create_model():
        return SSDamilRClassifierV27(
            embedding_dim=embedding_dim,
            num_ling_features=16,
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        # Normalize linguistic features (fit on train only)
        train_data, ling_mean, ling_std = normalize_ling_features(train_data, fit=True)
        val_data, _, _ = normalize_ling_features(val_data, mean=ling_mean, std=ling_std, fit=False)
        test_data_norm, _, _ = normalize_ling_features(copy.deepcopy(test_set), mean=ling_mean, std=ling_std, fit=False)

        train_ds = DualRoleBagDataset(train_data, instance_dropout=args.instance_dropout)
        val_ds = DualRoleBagDataset(val_data)
        test_ds = DualRoleBagDataset(test_data_norm)
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, _ = train_epoch_v27(
            ...
            v_l, v_ml, v_metrics, v_preds = evaluate_v27(model, val_loader, criterion, device)
        ...
        if len(swa) >= 3:
            ...
            _, _, best_val_metrics, _ = evaluate_v27(model, val_loader, criterion, device)
            best_val_score = best_val_metrics.get("roc_auc", 0.0)

            swa_model = _create_model()
            swa.apply(swa_model)
            _, _, swa_val_metrics, _ = evaluate_v27(swa_model, val_loader, criterion, device)
        ...
        _, _, v_m, v_p = evaluate_v27(model, val_loader, criterion, device)
        best_t = find_best_threshold(
            np.array(v_p["true_label"]), np.array(v_p["probability"]),
            metric="loss", pos_weight=neg_to_pos,
        )

        _, _, test_metrics, test_results = evaluate_v27(model, test_loader, criterion, device, threshold=best_t)
        checkpoint_path.unlink(missing_ok=True)
        return {**test_metrics, **test_results}

    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v27 - Linguistic Fusion)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v27 (Linguistic Fusion) CV Results\n\n{report}")
    logger.info("\n" + report)
