"""SS-DAMIL-R v31: Manifold Mixup + SAM Optimizer.

Replaces standard AdamW with Sharpness-Aware Minimization (SAM) using AdamW as base.
SAM penalizes sharp minima, acting as an implicit ensemble and very strong regularizer,
perfect for the extremely small DAIC-WOZ dataset.
"""
...
# ---------------------------------------------------------------------------
# Training with Manifold Mixup & SAM
# ---------------------------------------------------------------------------
...
def train_epoch_v31(
    ...
    for batch in loader:
        ...
        # SAM Step 1: First forward-backward to find adversarial perturbation
        loss, ml, al, dl, logits = forward_pass()
        ...
        # SAM Step 2: Second forward-backward at adversarial point
        loss_adv, _, _, _, _ = forward_pass()
        loss_adv.backward()
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v31: Manifold Mixup + SAM")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v31")
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        optimizer = SAM(model.parameters(), base_optimizer=torch.optim.AdamW, lr=args.lr, weight_decay=1e-4, rho=args.sam_rho, adaptive=args.sam_adaptive)
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, _ = train_epoch_v31(
    ...
    logger.info("Starting Stratified Group K-Fold CV (v31 - SAM + Manifold Mixup)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v31 (SAM + Manifold Mixup) CV Results\n\n{report}")
    logger.info("\n" + report)
