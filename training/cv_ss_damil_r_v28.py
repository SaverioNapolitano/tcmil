"""SS-DAMIL-R v28: Mean Teacher Consistency.

Introduces Exponential Moving Average (EMA) Teacher consistency regularization.
By computing an MSE consistency loss between the Student's and Teacher's
unmixed predictions, the model is penalized for oscillating or making
brittle predictions, heavily stabilizing performance on the small dataset.
"""
...
def train_epoch_v28(
    model, ema_model, loader, criterion, optimizer, device,
    max_grad_norm=1.0, noise_std=0.0,
    mixup_alpha=0.2, ema_alpha=0.99, consistency_weight=1.0, global_step=0,
):
    model.train()
    ema_model.train()
    ...
    for batch in loader:
        ...
        # Step 2: EMA Teacher forward pass (no noise)
        with torch.no_grad():
            teacher_pooled, _ = ema_model.forward_batch_split(
                batch["patient_bags"].to(device),
                batch["interviewer_bags"].to(device),
                batch["patient_sizes"],
                batch["interviewer_sizes"],
            ...
            teacher_head = ema_model.forward_heads(teacher_pooled)
            teacher_logits = teacher_head["logits"]
            teacher_sym_logits = teacher_head["symptom_logits"]

        # Step 3: Consistency Loss
        consistency_loss = F.mse_loss(torch.sigmoid(logits), torch.sigmoid(teacher_logits))

        # Step 4: Classification Loss (with Manifold Mixup)
        ...
        # Step 5: EMA Update
        global_step += 1
        update_ema_variables(model, ema_model, ema_alpha, global_step)
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v28: Mean Teacher Consistency")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v28")
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        # Model (v9 architecture) + EMA Teacher
        model = _create_model()
        ema_model = _create_model()
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, _, global_step = train_epoch_v28(
                model, ema_model, train_loader, criterion, optimizer, device,
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v28 - Mean Teacher Consistency)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v28 (Mean Teacher Consistency) CV Results\n\n{report}")
    logger.info("\n" + report)
