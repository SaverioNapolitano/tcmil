"""SS-DAMIL-R v9c: Full v9 with Augmentation (ablation).

Builds on v9b (architecture + training improvements) and adds:
  - Embedding-space Mixup (post-pooling)
  - Increased instance dropout (0.15 -> 0.20)
  - Dynamic auxiliary weight scheduling (0.5 -> 0.1 over training)
"""
...
# ---------------------------------------------------------------------------
# Multi-Task Loss WITH Dynamic Aux Weight + Diversity
# ---------------------------------------------------------------------------

class DynamicMultiTaskDiversityLoss(nn.Module):
    """Main + Symptom + Diversity, with dynamic aux weight scheduling."""
    ...
    def forward(
        ...
        total_loss = main_loss + aux_w * avg_aux_loss + self.diversity_weight * div_loss
...
def train_epoch_v9c(
    ...
    """Training loop with post-pooling mixup augmentation."""
    model.train()
    ...
    for batch in loader:
        ...
        symptom_logits = output["symptom_logits"]

        # --- Embedding-Space Mixup ---
        # Mix logits and targets within the batch if batch_size >= 2
        if mixup_alpha > 0 and logits.size(0) >= 2:
            lam = torch.distributions.Beta(mixup_alpha, mixup_alpha).sample().item()
            # Create a shuffled permutation of the batch
            perm = torch.randperm(logits.size(0), device=device)

            # Mix logits (both main and symptom)
            mixed_logits = lam * logits + (1 - lam) * logits[perm]
            mixed_sym_logits = lam * symptom_logits + (1 - lam) * symptom_logits[perm]

            # Mix targets
            mixed_target = lam * target + (1 - lam) * target[perm]
            mixed_sym_target = lam * sym_target + (1 - lam) * sym_target[perm]
            mixed_has_sym = torch.maximum(has_sym, has_sym[perm])  # If either has symptoms
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v9c: Full v9 (Augmentation)")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v9c")
    ...
    parser.add_argument("--instance_dropout", type=float, default=0.20)  # INCREASED from 0.15
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        for epoch in range(1, args.max_epochs + 1):
            # v9c: Update dynamic aux weight
            criterion.set_epoch(epoch)

            avg_l, ml, al, dl, _ = train_epoch_v9c(
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v9c - Full)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v9c (Full v9: Augmentation) CV Results\n\n{report}")
    logger.info("\n" + report)
