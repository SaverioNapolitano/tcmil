"""SS-DAMIL-R v11: Full Combination (SupCon + R-Drop + Temporal).

Combines all three v11 strategies:
  1. Temporal-Aware Encoding (v11c model with positional encoding)
  2. Supervised Contrastive Regularization (v11a SupCon loss)
  3. R-Drop Consistency Regularization (v11b symmetric KL)

Uses the v11c model architecture (SSDamilRClassifierV11c) with both
SupCon and R-Drop losses during training. Batch size 16 for contrastive.
Dropout 0.4 for meaningful R-Drop perturbation.
"""
...
from training.cv_ss_damil_r_v11a import SupConBagLoss
...
# ---------------------------------------------------------------------------
# Training with SupCon + R-Drop + Temporal Encoding
# ---------------------------------------------------------------------------

def train_epoch_v11(
    model, loader, criterion, supcon_loss_fn, rdrop_loss_fn,
    optimizer, device,
    max_grad_norm=1.0, noise_std=0.0, con_weight=0.3,
):
    """Training loop combining SupCon and R-Drop on temporal-aware model.

    Each batch:
    1. Two forward passes through backbone → pooled representations (R-Drop)
    2. Classification heads on both passes
    3. Classification loss averaged over both passes
    4. SupCon loss on pooled representations (pass 1)
    5. R-Drop KL divergence between the two output distributions
    """
    model.train()
    total_loss, total_m_loss, total_a_loss, total_d_loss = 0, 0, 0, 0
    total_c_loss, total_r_loss = 0, 0
    ...
    for batch in loader:
        ...
        # Pass 1: backbone → pooled reps → heads
        pooled_1, div_loss_1 = model.forward_batch_split(
            patient_bags, interviewer_bags, p_sizes, i_sizes,
            noise_std=noise_std,
        )
        head_1 = model.forward_heads(pooled_1)

        # Pass 2: different dropout mask
        pooled_2, div_loss_2 = model.forward_batch_split(
            patient_bags, interviewer_bags, p_sizes, i_sizes,
            noise_std=noise_std,
        )
        head_2 = model.forward_heads(pooled_2)

        div_loss = (div_loss_1 + div_loss_2) / 2

        # Classification loss on both passes (averaged)
        loss_1, ml_1, al_1, dl_1 = criterion(
            head_1["logits"], target,
            head_1["symptom_logits"], sym_target, has_sym,
            diversity_loss=div_loss,
        )
        loss_2, ml_2, al_2, dl_2 = criterion(
            head_2["logits"], target,
            head_2["symptom_logits"], sym_target, has_sym,
            diversity_loss=div_loss,
        )
        ...
        # SupCon loss on pooled representations (pass 1)
        con_loss = supcon_loss_fn(pooled_1, target)

        # R-Drop consistency loss
        rdrop_loss = rdrop_loss_fn(head_1["logits"], head_2["logits"])

        # Total loss
        loss = cls_loss + con_weight * con_loss + rdrop_loss
        ...
        probs = torch.sigmoid(head_1["logits"]).detach().cpu().numpy()
    ...
    return (
        ...
        total_c_loss / n_batches, total_r_loss / n_batches,
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v11: Full Combination")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v11")
    ...
    # Training Config
    parser.add_argument("--dropout_rate", type=float, default=0.4)  # Higher for R-Drop
    ...
    parser.add_argument("--batch_size", type=int, default=16)  # Larger for SupCon
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        for epoch in range(1, args.max_epochs + 1):
            avg_l, ml, al, dl, cl, rl, _ = train_epoch_v11(
                model, train_loader, criterion, supcon_loss_fn, rdrop_loss_fn,
                optimizer, device,
            ...
            if epoch % 10 == 0 or epoch == 1:
                ...
                logger.info(
                    ...
                    f"L:{avg_l:.4f} (M:{ml:.4f}, A:{al:.4f}, D:{dl:.4f}, C:{cl:.4f}, R:{rl:.4f}) | "
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v11 - Full)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v11 (Full: SupCon + R-Drop + Temporal) CV Results\n\n{report}")
    logger.info("\n" + report)
