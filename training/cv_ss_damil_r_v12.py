"""SS-DAMIL-R v12: Manifold-R-Drop & Symptom-Guided Cross-Attention.

Combines the architectural improvements of V12 (Symptom-Guided Cross-Attention) 
with the V9d Manifold Mixup and V11b R-Drop Consistency training techniques.
"""
...
from models.ss_damil_r import SSDamilRClassifierV12
...
from training.cv_ss_damil_r_v11b import RDropLoss
...
# ---------------------------------------------------------------------------
# Training with MR-Drop (Manifold Mixup + R-Drop)
# ---------------------------------------------------------------------------
...
def train_epoch_v12(
    ...
    for batch in loader:
        ...
        # Run backbone TWICE for R-Drop
        pooled_1, dl_1 = model.forward_batch_split(
            patient_bags, interviewer_bags, p_sizes, i_sizes, noise_std=noise_std
        )
        pooled_2, dl_2 = model.forward_batch_split(
            patient_bags, interviewer_bags, p_sizes, i_sizes, noise_std=noise_std
        )

        # Run heads on original pooled representations (clean)
        clean_out_1 = model.forward_heads(pooled_1)
        clean_out_2 = model.forward_heads(pooled_2)
        logits_1 = clean_out_1["logits"]
        logits_2 = clean_out_2["logits"]

        if mixup_alpha > 0 and pooled_1.size(0) >= 2:
            # Same Mixup for both passes
            lam = torch.distributions.Beta(mixup_alpha, mixup_alpha).sample().item()
            perm = torch.randperm(pooled_1.size(0), device=device)

            mixed_1 = lam * pooled_1 + (1 - lam) * pooled_1[perm]
            mixed_2 = lam * pooled_2 + (1 - lam) * pooled_2[perm]
            ...
            mixed_out_1 = model.forward_heads(mixed_1)
            mixed_out_2 = model.forward_heads(mixed_2)

            mlogits_1, msym_logits_1 = mixed_out_1["logits"], mixed_out_1["symptom_logits"]
            mlogits_2, msym_logits_2 = mixed_out_2["logits"], mixed_out_2["symptom_logits"]

            loss_1, m_l_1, a_l_1, d_l_1 = criterion(
                mlogits_1, mixed_target, msym_logits_1, mixed_sym_target, mixed_has_sym, diversity_loss=dl_1
            )
            loss_2, m_l_2, a_l_2, d_l_2 = criterion(
                mlogits_2, mixed_target, msym_logits_2, mixed_sym_target, mixed_has_sym, diversity_loss=dl_2
            )

            # MR-Drop Loss: R-Drop on mixed logic + R-Drop on clean logic
            mixed_rdrop = rdrop_loss_fn(mlogits_1, mlogits_2)
            clean_rdrop = rdrop_loss_fn(logits_1, logits_2)
            rdrop_loss = (mixed_rdrop + clean_rdrop) / 2
            ...
            loss_1, m_l_1, a_l_1, d_l_1 = criterion(
                logits_1, target, clean_out_1["symptom_logits"], sym_target, has_sym, diversity_loss=dl_1
            )
            loss_2, m_l_2, a_l_2, d_l_2 = criterion(
                logits_2, target, clean_out_2["symptom_logits"], sym_target, has_sym, diversity_loss=dl_2
            )

            cls_loss = (loss_1 + loss_2) / 2
            rdrop_loss = rdrop_loss_fn(logits_1, logits_2)
        ...
        # Track metrics on original (clean) pass 1 predictions
        probs = torch.sigmoid(logits_1).detach().cpu().numpy()
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v12: MR-Drop + Cross-Attention")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v12")
    ...
    parser.add_argument("--instance_dropout", type=float, default=0.15)  # Back to 0.15 for balance
    ...
    parser.add_argument("--rdrop_alpha", type=float, default=0.5)        # 0.5 to balance with mixup
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, rl, _ = train_epoch_v12(
    ...
    logger.info("Starting Stratified Group K-Fold CV (v12 - MR-Drop & Cross-Attention)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v12 (Symptom-Gated Cross-Attention + MR-Drop) CV Results\n\n{report}")
    logger.info("\n" + report)
