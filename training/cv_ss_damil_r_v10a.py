"""SS-DAMIL-R v10a: Loss Normalization + Orthogonal Init (ablation).

Addresses the gradient dominance issue identified in v9d:
  - GradNorm-lite: Normalizes aux loss relative to main loss magnitude
    before applying the dynamic weight. This prevents the 10x gradient
    imbalance observed in v9d (aux_loss ~0.6 vs main_loss ~0.07).
  - Orthogonal head initialization: MultiHeadAttentionPooling heads start
    with orthogonal W/v, so diversity is immediate (not after warmup).
  - Reduced warmup to 3 epochs (from 5): normalized loss stabilizes faster.
  - All other v9d components retained: manifold mixup, SWA, cosine+warmup,
    dynamic aux scheduling
"""
...
# ---------------------------------------------------------------------------
# GradNorm-lite Multi-Task Loss
# ---------------------------------------------------------------------------

class NormalizedMultiTaskDiversityLoss(nn.Module):
    """Multi-task loss with gradient-magnitude normalization.

    Instead of simply weighting: L = L_main + w * L_aux,
    we normalize: L = L_main + w * (L_main.detach() / L_aux.detach()) * L_aux

    This ensures that the gradient contribution from the aux task is always
    proportional to the main task, regardless of raw loss magnitudes.
    """
    ...
    @property
    def current_aux_weight(self) -> float:
        """Linear decay from start to end over schedule_epochs."""
        progress = min(1.0, self._current_epoch / max(1, self.schedule_epochs))
        return self.aux_weight_start + progress * (self.aux_weight_end - self.aux_weight_start)
    ...
    def forward(
        ...
        div_loss = diversity_loss if diversity_loss is not None else torch.tensor(0.0, device=logits.device)

        # GradNorm-lite: normalize aux loss to match main loss magnitude
        # This prevents the ~10x gradient imbalance (aux ~0.6 vs main ~0.07)
        eps = 1e-8
        scale_factor = main_loss.detach() / (avg_aux_loss.detach() + eps)
        normalized_aux = scale_factor * avg_aux_loss

        aux_w = self.current_aux_weight
        total_loss = main_loss + aux_w * normalized_aux + self.diversity_weight * div_loss

        return total_loss, main_loss, avg_aux_loss, div_loss


# ---------------------------------------------------------------------------
# Training with Manifold Mixup (same as v9d)
# ---------------------------------------------------------------------------

def train_epoch_v10a(
    model, loader, criterion, optimizer, device,
    max_grad_norm=1.0, noise_std=0.0,
    mixup_alpha=0.2,
):
    """Training loop with manifold mixup + normalized loss."""
    model.train()
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v10a: Loss Normalization")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v10a")
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, _ = train_epoch_v10a(
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v10a - Loss Normalization)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v10a (Loss Normalization) CV Results\n\n{report}")
    logger.info("\n" + report)
