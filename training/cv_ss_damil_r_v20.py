...
Replaces v9c's post-pooling logit mixup with true Manifold Mixup:
  - Mixup applied at the pooled representation level (after multi-head
    attention pooling, BEFORE the classification heads)
  - This creates virtual training bags by interpolating in the learned
    manifold rather than mixing raw outputs
  - Keeps all v9c improvements: dynamic aux weight, increased instance
    dropout, SWA, cosine+warmup scheduler, label smoothing
"""
...
from models.ss_damil_r_v20 import SSDamilRClassifierV20
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v20: Dual-Path Distillation Pipeline")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v20")
    ...
    parser.add_argument("--prepend_roles", action="store_true", default=True)
    ...
    all_interviews = precompute_dual_role_embeddings(all_interviews, tokenizer, base_encoder, device, max_len=args.max_len, prepend_roles=args.prepend_roles)

    def _create_model():
        return SSDamilRClassifierV20(
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, _ = train_epoch_v20(
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v20 - Dual-Path)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v20 (Dual-Path Pooling) CV Results\n\n{report}")
    logger.info("\n" + report)
