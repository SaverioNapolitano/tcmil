"""SS-DAMIL-R v13: Temporal Context, Gated Attention Pooling, and Manifold Mixup.

Incorporates:
- Transformer Encoder applied over the features of the turns (Temporal sequence).
- Gated Multi-Head Attention Pooling (Ilse et al. 2018).
- Gated Symptom Injection (from v9).
- Manifold Mixup (from v9d) acting on the pooled representation.
"""
...
from models.ss_damil_r import SSDamilRClassifierV13
...
def train_epoch_v13(
    ...
    for batch in loader:
        ...
        logits = head_output["logits"] if "logits" in head_output else head_output["logit"]
        ...
        if mixup_alpha > 0 and pooled_batch.size(0) >= 2:
            ...
            mixed_logits = mixed_head_output["logits"] if "logits" in mixed_head_output else mixed_head_output["logit"]
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v13: Temporal Context (BiGRU) + Gated Pooling")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v13")
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, _ = train_epoch_v13(
            ...
            if epoch % 10 == 0 or epoch == 1:
                ...
                logger.info(
                    ...
                    f"Val L:{v_l:.4f} | F1:{v_metrics.get('f1', 0):.4f} | "
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v13 - Temporal Context + Gated Pooling)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v13 (Temporal Context + Gated Pooling) CV Results\n\n{report}")
    logger.info("\n" + report)
