"""SS-DAMIL-R v32: Manifold Mixup + FGM Adversarial Training.

Builds upon v9d by adding Fast Gradient Method (FGM) adversarial training.
FGM explicitly forces the model's manifold space to be robust to small semantic
shifts by adding adversarial perturbations to the input embeddings (patient/interviewer bags)
in the direction of the gradient. 
- Normal pass: Manifold Mixup is applied.
- Adversarial pass: Unmixed, perturbed embeddings are used for a clean robustness signal.
"""
...
def train_epoch_v32(
    ...
    mixup_alpha=0.2, fgm_epsilon=1.0,
):
    ...
    for batch in loader:
        ...
        # We must require_grad on the embeddings to compute FGM perturbations
        patient_bags = batch["patient_bags"].to(device).requires_grad_(True)
        interviewer_bags = batch["interviewer_bags"].to(device).requires_grad_(True)
        ...
        # ---------------------------------------------------------
        # PASS 1: Normal Forward (with Manifold Mixup)
        # ---------------------------------------------------------
        ...
        # ---------------------------------------------------------
        # PASS 2: Adversarial Forward (FGM)
        # ---------------------------------------------------------
        if fgm_epsilon > 0:
            p_grad = patient_bags.grad.detach()
            i_grad = interviewer_bags.grad.detach()

            p_norm = torch.norm(p_grad, p=2)
            i_norm = torch.norm(i_grad, p=2)

            r_p = fgm_epsilon * p_grad / (p_norm + 1e-8)
            r_i = fgm_epsilon * i_grad / (i_norm + 1e-8)

            # Create adversarial inputs
            patient_bags_adv = (patient_bags + r_p).detach()
            interviewer_bags_adv = (interviewer_bags + r_i).detach()

            # Forward with adversarial inputs (NO Mixup to maximize robustness signal)
            pooled_batch_adv, diversity_loss_adv = model.forward_batch_split(
                patient_bags_adv,
                interviewer_bags_adv,
                batch["patient_sizes"],
                batch["interviewer_sizes"],
                noise_std=noise_std,
            )

            head_output_adv = model.forward_heads(pooled_batch_adv)
            
            loss_adv, _, _, _ = criterion(
                head_output_adv["logits"], target,
                head_output_adv["symptom_logits"], sym_target, has_sym,
                diversity_loss=diversity_loss_adv,
            )
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v32: Manifold Mixup + FGM")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v32")
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, _ = train_epoch_v32(
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v32 - FGM + Manifold Mixup)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v32 (FGM + Manifold Mixup) CV Results\n\n{report}")
    logger.info("\n" + report)
