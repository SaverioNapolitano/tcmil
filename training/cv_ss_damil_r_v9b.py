"""SS-DAMIL-R v9b: Architecture Changes (ablation).

Uses the new v9 model architecture (multi-head pooling + gated symptom injection)
with all v9a training improvements:
  - Cosine Annealing with Warm Restarts + Linear warmup
  - Stochastic Weight Averaging (SWA)
  - Label smoothing
  - Diversity loss regularization for multi-head pooling
"""
...
# ---------------------------------------------------------------------------
# Multi-Task Loss WITH Diversity Loss
# ---------------------------------------------------------------------------

class MultiTaskDiversityLoss(nn.Module):
    """Main (Focal) + Symptom (Weighted BCE) + Diversity (cosine similarity)."""
    ...
    def forward(
        ...
        # 3. Diversity Loss (encourage multi-head to attend differently)
        div_loss = diversity_loss if diversity_loss is not None else torch.tensor(0.0, device=logits.device)

        total_loss = main_loss + self.aux_weight * avg_aux_loss + self.diversity_weight * div_loss

        return total_loss, main_loss, avg_aux_loss, div_loss


# ---------------------------------------------------------------------------
# Train / Evaluate adapted for v9 (diversity loss)
# ---------------------------------------------------------------------------
...
def evaluate_v9(model, loader, criterion, device, threshold=0.5):
    model.eval()
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v9b: Architecture Changes")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v9b")
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v9b)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v9b (Architecture Changes) CV Results\n\n{report}")
    logger.info("\n" + report)
