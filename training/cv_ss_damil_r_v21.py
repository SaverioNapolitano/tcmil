"""SS-DAMIL-R v21: Corrected R-Drop + Manifold Mixup.

Combines the EXACT v9d architecture (multi-head pooling + gated symptom injection)
with Manifold Mixup and R-Drop Consistency Regularization.

Differs from v19:
- Fixes the loss scale issue: R-Drop is applied purely as a regularization penalty
  (Loss = L_mix + R_kl) without duplicatively running the classification loss 
  on unmixed instances, which caused gradient magnitude issues in v19.
- Removes MC-Dropout at evaluation to restore well-calibrated confidence boundaries.
"""
...
# ---------------------------------------------------------------------------
# Training with Manifold Mixup + R-Drop (Corrected)
# ---------------------------------------------------------------------------
...
def train_epoch_v21(
    ...
    for batch in loader:
        ...
        # 3. R-Drop Consistency — run unmixed representations TWICE (act purely as regularization)
        out_1 = model.forward_heads(pooled_batch)
        out_2 = model.forward_heads(pooled_batch)

        R_kl = rdrop_loss_fn(out_1["logits"], out_2["logits"])

        # 4. Total Loss - Corrected (Doesn't add unmixed classification loss to the total loss scale)
        loss = L_mix + R_kl
        ...
        total_m_loss += mix_ml.item()
        total_a_loss += mix_al.item()
...
def main():
    parser = argparse.ArgumentParser(description="SS-DAMIL-R v21: Corrected R-Drop + Manifold Mixup")
    parser.add_argument("--data_dir", type=str, default="data")
    parser.add_argument("--output_dir", type=str, default="results/ss_damil_r_cv_v21")
    ...
    def train_eval_fn(train_pool, test_set, run_seed):
        ...
        for epoch in range(1, args.max_epochs + 1):
            ...
            avg_l, ml, al, dl, rl, _ = train_epoch_v21(
            ...
            # Using regular evaluate_v9 for robust evaluation metrics
    ...
    # --- Run CV ---
    logger.info("Starting Stratified Group K-Fold CV (v21 - Corrected R-Drop + Mixup)...")
    ...
    with open(out_dir / "cv_report.txt", "w") as f:
        f.write(f"# SS-DAMIL-R v21 (Corrected R-Drop + Mixup) CV Results\n\n{report}")
    logger.info("\n" + report)
